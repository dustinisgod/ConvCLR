...
local clericName = mq.TLO.Me.Name()
local configPath = mq.configDir .. '/' .. clericName .. '_config.lua'
local config = {}
...
local function clericControlGUI()
    ...
    if gui.mainAssist ~= "" then
        ImGui.SetNextItemWidth(100)
        gui.assistRange = ImGui.SliderInt("Assist Distance", gui.assistRange, 5, 100)
    end
    ImGui.Spacing()
    ImGui.Separator()
    ImGui.Spacing()
    ColoredText("Heal Settings", {1.0, 1.0, 0.0, 1.0})
    ImGui.Separator()
    ...
    ColoredText("Res Settings", {1.0, 1.0, 0.0, 1.0})
    ImGui.Separator()
    ...
    ImGui.Separator()
    ColoredText("Nav Settings:", {1.0, 1.0, 0.0, 1.0})
    ImGui.Separator()
    ...
    if gui.chaseOn then
        ...
        ImGui.Separator()
        ColoredText("Misc Settings", {1.0, 1.0, 0.0, 1.0})
        ImGui.Separator()
